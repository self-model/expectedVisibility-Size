...
for s = 1:nsubjects
    ...
    for i = 1:numRows
        occlusion_index = df_filtered.easy(i)+1;
        present_index = df_filtered.present(i)+1;
        decision_index = df_filtered.decision(i)+1;
        rt_index = df_filtered.rt(i)+1;
        subj_data(s,occlusion_index,present_index,decision_index,rt_index)=...
        subj_data(s,occlusion_index,present_index,decision_index,rt_index)+1;
    end
end
...
for seed=1:nseeds
    ...
    save('E_occ_fit_local','fitted_parameters', 'LL', 'subj_data');
end
...
save('E_occ_fit_local','fitted_parameters', 'LL', 'subj_data', 'best_parameters');
